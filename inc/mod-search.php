...
const RD_SEARCH_LAYOUT_ORDER = [ 'grid', 'vertical', 'compact', 'google' ];
...
function rd_search_get_admin_active_layouts() : array {
    ...
    return ! empty( $active ) ? $active : [ 'compact' ];
}
...
function rd_search_localize_script() {
    ...
    wp_localize_script( 'rd-navigation', 'rd_search_data', [
...
function rd_ajax_search_redistribute() {
    ...
    $requested = $active_csv ? explode( ',', $active_csv ) : [];
