...
/**
 * Network catalog: slug => display label, in the canonical display order.
 * Single source for every consumer — the footer/top-bar icon row, the author
 * profile fields and the RD_Social_Widget ("Follow Us").
 */
function rd_social_get_networks(): array {
	return array(
		'discord'   => 'Discord',
		'telegram'  => 'Telegram',
		'whatsapp'  => 'WhatsApp',
...
/**
 * Inline SVG icon library (slug => <svg>). Extracted from
 * rd_render_social_icons() so the social widget reuses the exact same icons.
 */
function rd_social_get_icons(): array {
...
function rd_render_social_icons( $user_id = null ) {
	$redes = array_keys( rd_social_get_networks() );
	$svgs  = rd_social_get_icons();
