...
/**
 * Whether the current request maps to a template with a DEDICATED critical
 * file (the 5 generated by tools/critical). Unmapped contexts (404,
 * attachments, etc.) must NOT defer the main stylesheet: the home-fallback
 * critical doesn't cover their above-the-fold styles, so the page painted
 * naked until the async CSS landed (the "F5 fixes it" symptom on the 404).
 * On those pages the stylesheet stays synchronous — correctness over a
 * micro-optimization for rare page types.
 */
function rd_critical_css_is_covered(): bool {
	return is_singular( 'post' ) || is_page() || is_search()
		|| is_archive() || is_category() || is_tag() || is_author() || is_date() || is_tax()
...
function rd_critical_css_inject_inline() {
	...
	if ( ! rd_critical_css_is_covered() ) {
		return; // 404/attachment etc.: no useful critical to inline (see helper)
	}
...
function rd_critical_css_defer_main_stylesheet( $tag, $handle ) {
	...
	// Uncovered template (404, attachment, ...): keep the stylesheet
	// synchronous. The old guard below was defeated here by the home-fallback
	// in rd_critical_css_get_content() (non-empty content → deferred anyway,
	// with a critical that doesn't style the page → naked first paint).
	if ( ! rd_critical_css_is_covered() ) {
		return $tag;
	}
