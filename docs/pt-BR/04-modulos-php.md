...
**Interruptor mestre (`enable_ads`, default ON)** — `rd_ads_enabled()` porteia **toda** saída de anúncio: os 3 banners, o widget, as zonas in-article, o script global (`ad_global` em mod-integrations) e o bootstrap lazy. Desligado, **nada de ad renderiza** e o `rd_csp_ads_active()` retorna false (a CSP dropa as origens de ad). Os códigos ficam **salvos no painel** — pausa/retomada de 1 clique. O `ads.txt` continua servindo (é declaração, não anúncio).

**Anúncio in-article (`rd_ads_inject_in_article`, filtro `the_content` prio 30)** — alternativa controlada ao Auto Ads: injeta uma zona de anúncio **após o N-ésimo `<p>` de primeiro nível** dos single posts (não páginas), com `min-height` reservado no container = **zero CLS**. Até **2 zonas** (`ad_in_article` / `ad_in_article_2`), cada uma com parágrafo + altura próprios; a #2 só dispara quando o post é longo o bastante pra alcançar o parágrafo dela (qualifica em N+1 parágrafos). Usa **DOMDocument** contando só os `<p>` filhos diretos (parágrafos em blockquote/lista/code são pulados), com **placeholder swap** (preserva o `<script>` do ad, que o DOMDocument corromperia). Reusa `rd_csp_inject_nonce` + `rd_ads_dedupe_loader` (CSP + lazy automáticos). **Idempotente** contra dupla aplicação do `the_content` (o TOC reaplica os filtros pra extrair headings, assando a zona no `modified_content` cacheado) — checa o marcador `rd-ad-in-article` e não reinjeta. Prio 30 = depois de markdown(6)/imagens(20)/TOC(25), na estrutura final.
