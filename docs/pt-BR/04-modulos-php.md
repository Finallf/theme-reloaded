...
- **Preload Critical Fonts** (`preload_critical_fonts`, default ON): injeta `<link rel="preload" as="font" type="font/woff2" crossorigin>` pra **5 variantes críticas** no `wp_head` priority 1: **Inter Regular**, **Inter Medium (500)**, **Inter Bold (700)**, **Poppins SemiBold (600)** e **Poppins Bold (700)**. Lista validada por auditoria do Network (cache desabilitado) — todas aparecem em prioridade "Highest" assim que o browser parseia o `style.css`. Preload antecipa o download em paralelo, reduzindo FOIT/FOUT no first paint. Outras variantes que o CSS também eleva pra "Highest" (poppins-500/800, inter-600/italic, jetbrains-mono-regular) ficaram **fora** propositalmente — auditoria pendente no Background do BACKLOG vai decidir se são realmente above-the-fold ou se o SCSS pode reduzir o uso pra cortar carga inicial
- **Fallbacks com métricas casadas** (`_variables.scss`): faces sintéticas `Inter-fallback`/`Poppins-fallback` (`src: local('Arial')` + `size-adjust`/`ascent`/`descent`/`line-gap-override`, valores da calibração fontaine) entram nos stacks logo após a fonte real. Com `font-display: swap`, o texto pinta no Arial **redimensionado pra ocupar as mesmas caixas** da fonte final — o swap fica geometricamente invisível (era a fonte do CLS residual: banner LGPD 0,033-0,065 + respingos). Browser sem `size-adjust` ignora os descritores — comportamento idêntico ao anterior.
